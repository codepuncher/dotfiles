...
test_cache() {
	...
	# build is a no-op when the cache already exists: a new project must NOT appear.
	mkdir -p "${ROOT}/mods/NewMod/.git"
	_project_open_build_cache
	assert_eq "build skips when cache exists" "no" \
		"$(grep -q '/NewMod$' "${cache}" && echo yes || echo no)"

	# po_refresh forces a rebuild and picks up the new project.
	po_refresh
	...
	# Check the cache path for the missing root itself, since cache files are
	# keyed by root: building under a missing root must not create its cache.
	local missing_cache
	missing_cache="$(PROJECT_OPEN_ROOT="${ROOT}/missing" _project_open_cache_file)"
	PROJECT_OPEN_ROOT="${ROOT}/missing" _project_open_build_cache
	assert_eq "no cache built when root missing" "no" \
		"$([[ -f "${missing_cache}" ]] && echo yes || echo no)"
}
